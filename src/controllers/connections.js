...
module.exports.getConnectionRequests = createController(async (req, res) => {
  ...
  const { error, result } = await executeQuery(
    req.app.locals.db,
    `EXEC proc_get_user_connections_requests ${userId}`
  );
  ...
  console.log("Connections -> ", finalResult);
...
module.exports.getConnectionStatus = createController(async (req, res) => {
  ...
  const connection = result.find(
    (item) =>
      item.fromUserId === Number(userId) || item.toUserId === Number(userId)
  );
  ...
  if (
    connection.fromUserId === Number(userId) ||
    connection.toUserId === Number(userId)
  ) {
    res.status(200).send({
      data: {
